...
- [ ] T001 Create solution `EgyptTax.sln` at repository root and add empty .csproj files for all 5 src projects + 4 test projects per [plan.md](plan.md) §"Project Structure"
- [ ] T002 [P] Add `Directory.Build.props` at `src/Directory.Build.props` enforcing C# 12, .NET 8, nullable enabled, treat warnings as errors, and shared analyzers
- [ ] T003 [P] Add `Directory.Packages.props` at repo root with `ManagePackageVersionsCentrally` and pin every NuGet version listed in [plan.md](plan.md) §"Primary Dependencies"
- [ ] T004 [P] Add `.editorconfig` at repo root with .NET formatting + Roslyn analyzers configuration
- [ ] T005 [P] Add `.gitignore` rules covering `bin/`, `obj/`, `appsettings.Development.json`, `*.user`, `EgyptTax-Dev/`, `coverage/`, `playwright-report/`
- [ ] T006 Create solution folders + project references: SharedKernel ← Domain ← Application ← Infrastructure ← Web; Installer references Web; tests reference all production projects per Clean Architecture
- [ ] T007 [P] Add `dotnet-tools.json` manifest with `dotnet-ef` (8.x) and `csharpier`; document `dotnet tool restore` in [quickstart.md](quickstart.md)
- [ ] T008 [P] Configure GitHub Actions workflow at `.github/workflows/ci.yml` running on `windows-2022`: restore → build → test (all 4 projects) → wix build (release branch only) per [research.md](research.md) R-24
- [ ] T009 [P] Add `serilog.json` configuration template under `src/EgyptTax.Web/` for operational logging per R-20
- [ ] T010 [P] Create `docs/` directory with placeholder operator-runbook.md, accountant-guide.md, inspector-bundle-format.md
- [ ] T011 [P] Create empty `migrations/` directory with README documenting EF Core migration commands
- [ ] T012 Verify `dotnet build -c Release` produces no warnings; capture as the Setup-phase exit gate
...
- [ ] T023 Implement `AppDbContext` at `src/EgyptTax.Infrastructure/Persistence/AppDbContext.cs` with empty model + connection string binding from configuration
- [ ] T024 Configure value-object EF Core conversions for `MoneyEgp`, `ArabicEnglishText`, `EgyptianTin` at `src/EgyptTax.Infrastructure/Persistence/ValueObjectConversions.cs`
- [ ] T025 Generate and apply initial EF migration `Initial` (creates DB + audit/audit_meta schemas) at `migrations/`
- [ ] T026 [P] Add Testcontainers SQL Server fixture at `tests/EgyptTax.IntegrationTests/Infrastructure/SqlServerFixture.cs`
- [ ] T027 [P] Add EF Core migration smoke test at `tests/EgyptTax.IntegrationTests/Persistence/MigrationSmokeTests.cs` — applies all migrations on a fresh container

### Audit log + hash chain (FR-028, FR-042)

- [ ] T028 Implement `AuditLogEntry` entity at `src/EgyptTax.Domain/Audit/AuditLogEntry.cs` per data-model F1
- [ ] T029 Implement `AuditChainHasher` (SHA-256 over canonicalized payload + prev_hash) at `src/EgyptTax.Domain/Audit/AuditChainHasher.cs`
- [ ] T030 Implement `SqlAuditLogStore` (append-only, monotonic index allocator) at `src/EgyptTax.Infrastructure/Audit/SqlAuditLogStore.cs`
- [ ] T031 Add SQL migration adding INSERT-only trigger blocking UPDATE/DELETE on `audit.audit_log` for the application's service account, at `migrations/[seq]_AuditAppendOnlyTrigger.cs`
- [ ] T032 Implement `IAuditCheckpointStore` abstraction at `src/EgyptTax.Application/Audit/IAuditCheckpointStore.cs` and two implementations (`FileSystemCheckpointStore`, `SqlSchemaCheckpointStore`) at `src/EgyptTax.Infrastructure/Audit/` per R-05
- [ ] T033 Implement `AuditChainVerifier` at `src/EgyptTax.Domain/Audit/AuditChainVerifier.cs` per [contracts/audit-chain-verifier.md](contracts/audit-chain-verifier.md) algorithm
- [ ] T034 [P] Integration test: audit append + chain verify clean log at `tests/EgyptTax.IntegrationTests/Audit/AuditChainHappyPathTests.cs` — MUST FAIL FIRST
- [ ] T035 [P] Integration test: 8 tamper cases from [contracts/audit-chain-verifier.md](contracts/audit-chain-verifier.md) §"Required tests" at `tests/EgyptTax.IntegrationTests/Audit/AuditChainTamperTests.cs` — MUST FAIL FIRST
- [ ] T036 [P] Integration test: 1,000,000-entry chain verifies in < 30 s (SC-010) at `tests/EgyptTax.IntegrationTests/Audit/AuditChainPerformanceTests.cs` — MUST FAIL FIRST
...
- [ ] T044 Implement `User`, `Role`, `Permission` entities at `src/EgyptTax.Domain/Identity/` per data-model A1–A3
- [ ] T045 Implement Argon2id `PasswordHasher` at `src/EgyptTax.Infrastructure/Identity/Argon2idPasswordHasher.cs` per R-08 (with format `{algo}${params}${salt}${hash}`)
- [ ] T046 Implement `TotpService` at `src/EgyptTax.Infrastructure/Identity/TotpService.cs` using Otp.NET per R-07; encrypt secret at rest using DPAPI
...
- [ ] T063 Implement `DocumentSeries` entity at `src/EgyptTax.Domain/Numbering/DocumentSeries.cs` and `DocumentNumberAllocator` table entity at `src/EgyptTax.Domain/Numbering/DocumentNumberAllocator.cs` per data-model D1, D2
- [ ] T064 Implement `SqlSequentialNumberAllocator` (UPDLOCK row-locked allocator inside the same transaction as the post) at `src/EgyptTax.Infrastructure/Numbering/SqlSequentialNumberAllocator.cs` per R-06
- [ ] T065 [P] Integration test: SC-006 — 500 concurrent posting attempts spanning a fiscal-year boundary, zero gaps / zero duplicates at `tests/EgyptTax.IntegrationTests/Numbering/ConcurrentPostingStressTests.cs` — MUST FAIL FIRST
- [ ] T066 [P] Integration test: rollback releases the number (subsequent successful post receives same number) at `tests/EgyptTax.IntegrationTests/Numbering/RollbackReleasesNumberTests.cs` — MUST FAIL FIRST

### Document state machine + immutability guards

- [ ] T067 Implement abstract `Document` base aggregate with FR-026 state machine at `src/EgyptTax.Domain/Documents/Document.cs`
- [ ] T068 Implement `DocumentTypeApprovalSetting` entity at `src/EgyptTax.Domain/Workflow/DocumentTypeApprovalSetting.cs`; seed Phase-1 default (sales invoices: approval disabled)
- [ ] T069 Implement `PostedDocumentImmutabilityGuard` (handler-level rejection of edit/delete/void on Posted) at `src/EgyptTax.Application/Common/Guards/PostedDocumentImmutabilityGuard.cs` per FR-027 / INV-002
- [ ] T070 [P] Unit test: every state transition (enabled and disabled approval modes) at `tests/EgyptTax.UnitTests/Domain/DocumentStateMachineTests.cs` — MUST FAIL FIRST
- [ ] T071 [P] Integration test: posted document rejects edit/delete/void per FR-027 at `tests/EgyptTax.IntegrationTests/Documents/ImmutabilityTests.cs` — MUST FAIL FIRST
