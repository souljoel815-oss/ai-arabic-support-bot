...
- [X] T025 [P] [US2] Expand `keywords_egy` for the 5 seed entries with Egyptian-dialect phrasings AND common Arabizi spellings of the same intents (e.g., "rag3", "shipping", "irga3") so retrieval fires on FR-007 inputs  *— done. All 5 seed entries carry 1–2 Arabizi forms in `keywords_egy` (e.g., 'cancel el order', 'ezzay aghyar el password', 'forgot password'). The 15 new T042 entries don't yet have Arabizi keywords; address opportunistically if FR-007 evaluations reveal misses on questions that should route to those new entries.*
