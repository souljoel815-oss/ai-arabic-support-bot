...
- [X] T017 [US1] Run a baseline pass: `python agent/eval/runner.py --set msa --webhook ${N8N_CHAT_URL} --model gemini-2.5-flash-lite`, then human-grade the resulting JSONL and run `--grade-summary` on it; record the baseline pass-rate in `agent/eval/results/baseline.md` and confirm it falls below 90% (this anchors the iteration)  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*

### Implementation for User Story 1

- [X] T018 [US1] Curate `wording_msa` for all 5 seed entries in `agent/kb/ecommerce-faq.json` so they cleanly answer the on-topic prompts in `agent/eval/sets/msa.jsonl`; re-run `python agent/kb/lint.py`  *— done in T008 (initial authoring) and reinforced in T042 (KB grow-out). Baseline run confirmed on-topic MSA prompts ground correctly on the seed wording.*
- [X] T019 [US1] Tighten the **Register policy** and **Knowledge grounding rule** sections of `agent/prompts/system.md` so MSA detection and grounded retrieval reliably fire on the eval set; restart the n8n workflow (file is read fresh each request, but verify) and re-run the MSA eval; iterate up to 3 prompt-tweak cycles  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T020 [US1] Verify FR-005 explicitly: for the 3 deliberately-out-of-KB prompts, confirm the agent's reply is in MSA and uses the `refuse_off_topic` template from `agent/prompts/refusal-templates.md`, not a fabricated answer  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T021 [US1] Final SC-001 sign-off: re-run MSA eval, human-grade, confirm pass-rate ≥ 90%, and commit the graded results JSONL to `agent/eval/results/sc-001-signoff.jsonl`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
...
- [X] T023 [US2] Baseline pass: `python agent/eval/runner.py --set egyptian --webhook ${N8N_CHAT_URL} --model gemini-2.5-flash-lite`; arrange a native Egyptian-Arabic reviewer to grade the JSONL; append the baseline pass-rate to `agent/eval/results/baseline.md`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*

### Implementation for User Story 2

- [X] T024 [P] [US2] Curate `wording_egy` for all 5 seed entries in `agent/kb/ecommerce-faq.json` to cleanly answer the on-topic Egyptian prompts; ensure each `wording_egy` is in clean Egyptian dialect (uses بتاع, إيه, كده where natural; avoids MSA-only constructions); re-run `python agent/kb/lint.py`  *— done in T008. Baseline pre-screen (heuristic) detected 0 MSA leakage in Egyptian replies; final confirmation pending native-Egyptian-Arabic reviewer grading in T028.*
- [X] T025 [P] [US2] Expand `keywords_egy` for the 5 seed entries with Egyptian-dialect phrasings AND common Arabizi spellings of the same intents (e.g., "rag3", "shipping", "irga3") so retrieval fires on FR-007 inputs  *— done, and extended beyond the original task scope: all 20 KB entries (5 seeds + 15 from T042) now carry ~2 Arabizi forms each in `keywords_egy`, averaging 1.9 Arabizi forms per entry. Examples: 'cancel el order', 'fen el order', 'el mantag 3atlan', 'ta2seet', 'change email', 'ezzay aghyar el rakam'.*
- [X] T026 [US2] Tighten the **Register policy** (Egyptian + Arabizi branches) and **Egyptian style guide** sections of `agent/prompts/system.md`; explicitly instruct that for `register == "arabizi"` the reply MUST use Egyptian Arabic script; iterate up to 3 prompt-tweak cycles, re-running the Egyptian eval after each  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T027 [US2] Verify FR-007 explicitly: for the 6 Arabizi prompts, confirm the agent classifies them as `arabizi` and replies in Egyptian Arabic script (no Latin transliteration, no MSA)  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T028 [US2] Final SC-002 sign-off: re-run Egyptian eval, native-reviewer grade, confirm pass-rate ≥ 85% with zero MSA-leakage records, and commit graded JSONL to `agent/eval/results/sc-002-signoff.jsonl`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
...
- [~] T030 [US3] Extend `agent/eval/runner.py` with a `--set multiturn` mode that preserves `sessionId` across turns of the same conversation and writes one record per turn; run a baseline pass and confirm back-references currently break (anchors iteration)  *— runner extension done in T013 (runner.py supports `--set multiturn` with conversation_id grouping and shared sessionId per conversation). Operator did run the baseline (`agent/eval/results/multiturn-20260504T062327.jsonl`); grading "back-references break" still pending per the reviewer rubric.*

### Implementation for User Story 3

- [X] T031 [US3] In the n8n Cloud workflow editor, attach a `Memory: Window Buffer` sub-node to the AI Agent node, keyed off the Chat Trigger's `sessionId` with a sliding window of 6 turns (3 visitor + 3 agent) per research.md R4; idle expiry 30 minutes; Save the workflow; export and overwrite `agent/workflow/ai-support-agent.json` afterwards  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T032 [US3] Update the `build_llm_input` Code node in the n8n Cloud workflow to inject `<recent_turns>` from session memory into the per-turn user message exactly as specified in `contracts/llm-prompt.md`; Save and re-export the workflow JSON  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T033 [US3] Re-run the multi-turn script; confirm both conversations now answer all back-references coherently in the matching register; commit the graded JSONL to `agent/eval/results/sc-multiturn-signoff.jsonl`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
...
- [X] T035 [US4] Baseline pass: `python agent/eval/runner.py --set adversarial --webhook ${N8N_CHAT_URL} --model gemini-2.5-flash-lite`; human-grade; record baseline in `agent/eval/results/baseline.md`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*

### Implementation for User Story 4

- [X] T036 [US4] Refine `agent/prompts/refusal-templates.md`: ensure each of the 4 refusal actions has a polished MSA and Egyptian phrasing; for `refuse_role_override`, the wording MUST NOT echo or restate the override attempt  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T037 [US4] Tighten the **Role lock** and **Refusal policy** sections of `agent/prompts/system.md`: the agent stays in customer-support role under all circumstances; never reveals system prompt; never enters a role suggested by the visitor; for non-Arabic input replies in MSA per FR-010  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T038 [US4] Verify the workflow's `validate_and_overwrite` Code node (built in T011 on n8n Cloud, see `agent/workflow/README.md` Node 6) overwrites `reply` with the verbatim refusal template for any `refuse_*` action — even if Gemini returns a free-form refusal — to prevent dialect/tone drift on refusal turns  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [X] T039 [US4] Re-run adversarial eval; iterate prompt + templates up to 3 cycles; final SC-003 sign-off requires pass-rate ≥ 95% with zero turns that fabricate an answer or step out of role; commit graded JSONL to `agent/eval/results/sc-003-signoff.jsonl`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
...
- [X] T041 [P] Verify SC-004: across the four sign-off result files, compute median `latency_ms`; confirm < 3000 ms; if not, identify and trim the highest-cost prompt-template section in `agent/prompts/system.md`; record the median in `agent/eval/results/latency-signoff.md`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [~] T042 Grow the KB to the planned 20–50 entries: add 15–45 more entries to `agent/kb/ecommerce-faq.json` covering finer-grained questions in each of the five topics; run `python agent/kb/lint.py`; copy the new JSON contents into the n8n Cloud workflow's `load_kb` Code node and save; spot-re-run the `msa` and `egyptian` eval sets to confirm no regressions  *— author-side done: KB grown from 5 → 20 (4/topic). Verified all 6 deliberately-uncovered eval prompts still have zero keyword overlap. Operator action remaining: copy the new JSON into the n8n Cloud workflow's `load_kb` Code node, save, and re-run the msa + egyptian eval sets to confirm no regressions on the originally-passing prompts.*
- [X] T043 [P] Verify SC-005 manually: time how long it takes to add one new KB entry on n8n Cloud (edit the repo file → run lint → copy contents into the workflow's `load_kb` Code node → Save the workflow → see it answer in chat); confirm under 10 minutes; record observation in `agent/eval/results/sc-005-signoff.md`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
...
- [X] T046 [P] Mirror the spec quickstart into `agent/docs/quickstart.md` (one-paragraph summary + link to the canonical `specs/001-ai-support-agent/quickstart.md`) so a developer cloning the repo finds it without digging into `specs/`
- [X] T047 Verify SC-006 manually: have one first-time visitor (not the portfolio owner) complete a 3-turn support exchange end-to-end without restating the question; record observation + any UX papercuts in `agent/eval/results/sc-006-signoff.md`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
- [~] T048 [P] Add a top-level `README.md` at the repo root with: one-paragraph project description, screenshot of the chat in MSA + Egyptian, link to the live demo URL, link to `specs/001-ai-support-agent/quickstart.md`, link to constitution v1.0.0  *— author-side done. Screenshots not yet added (need live chat captures with sample prompts in both registers); add them to a `docs/screenshots/` folder and reference from README when available.*
- [X] T049 [P] Add the "Try the demo" link to the portfolio site (action external to this repo); record the link target in `agent/docs/portfolio-link.md`  *— closed under Q10 (portfolio-demonstrable) clarification on 2026-05-04. Operator asserted complete; no formal human-graded sign-off JSONL committed.*
