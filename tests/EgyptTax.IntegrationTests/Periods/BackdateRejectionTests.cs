...
public class BackdateRejectionTests(SqlServerFixture fixture)
{
    ...
    public async Task Lock_Then_Backdate_Then_Reopen_Then_Post_Audits_Every_Step()
    {
        ...
        var postHandler = new PostSalesInvoiceHandler(
            db,
        ...
        var rejectAct = async () =>
            await postHandler.HandleAsync(
                new PostSalesInvoiceCommand(draft.Id, admin.Id),
                CancellationToken.None
            );
        await rejectAct
            .Should()
            ...
            .Where(ex =>
                ex.Message.Contains("FR-037", StringComparison.OrdinalIgnoreCase)
        ...
        await reopenHandler.ReopenAsync(
            new ReopenTaxPeriodCommand(
                ...
                5,
                admin.Id,
                "Late supplier invoice received — reopen for one post"
            ),
            CancellationToken.None
        );
        ...
        var postAfterReopen = new PostSalesInvoiceHandler(
            db,
            new SqlSequentialNumberAllocator(db),
            reopenClock,
            captureAudit,
