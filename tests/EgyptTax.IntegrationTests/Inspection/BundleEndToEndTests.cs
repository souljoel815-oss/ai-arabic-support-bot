...
public class BundleEndToEndTests(SqlServerFixture fixture) : IDisposable
{
    ...
    private readonly string _tempRoot = Path.Combine(
        Path.GetTempPath(),
        $"egypttax-bundle-perf-{Guid.NewGuid():N}"
    );
    ...
    private static async Task<(VatCategory Vat, Customer Customer, User User)> SeedMastersAsync(
        AppDbContext db
    )
    {
                    ...
                    var je = JournalEntry.Create(
                        ...
                        lines: new[]
                        {
                            (
                                ...
                                Credit: MoneyEgp.Zero,
                                Description: "AR"
                            ),
                            (
                                AccountCode: "4000",
                                Debit: MoneyEgp.Zero,
                                Credit: invoice.NetBeforeVat,
                                Description: "Revenue"
                            ),
                            (
                                AccountCode: "2110",
