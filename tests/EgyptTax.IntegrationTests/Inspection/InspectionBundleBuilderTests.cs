...
public class InspectionBundleBuilderTests(SqlServerFixture fixture) : IDisposable
{
    ...
    private readonly string _tempRoot = Path.Combine(
        Path.GetTempPath(), $"egypttax-bundle-{Guid.NewGuid():N}");
    ...
    public async Task HappyPath_Bundle_CarriesManifestAndAttachments_AndPerFileHashesMatch()
    {
        ...
        var draft = PurchaseInvoice.CreateDraft(supplier.Id, supplier.TaxProfile,
            "SUP-INV-1", new DateOnly(2026, 5, 5));
        draft.AddLine(itemId: null, expenseCategoryId: Guid.NewGuid(),
            ...
            deductibleFlag: true);
        db.Add(draft);
        var attachmentBytes = Encoding.UTF8.GetBytes("%PDF-1.7\nfake supplier invoice for hashing\n");
        ...
        db.Add(new Attachment(
            ...
            sha256: saved.ContentSha256, mimeType: "application/pdf",
        ...
        var postHandler = new PostPurchaseInvoiceHandler(db,
            new SqlSequentialNumberAllocator(db), clock, auditStore);
        ...
        result.Manifest.Files.Should().Contain(f => f.RelativePath == "MANIFEST.sha256" || f.Category == "ReadmeForInspector");
        result.Manifest.Files.Should().Contain(f => f.Category == "Attachment",
        ...
        foreach (var file in result.Manifest.Files)
        {
            ...
            entry.Should().NotBeNull(because: $"{file.RelativePath} listed in manifest but missing from ZIP");
            await using var s = entry!.Open();
            using var ms = new MemoryStream();
            await s.CopyToAsync(ms);
#pragma warning disable CA1308 // Manifest schema requires lowercase hex.
            var hashHex = Convert.ToHexString(SHA256.HashData(ms.ToArray()))
                .ToLowerInvariant();
#pragma warning restore CA1308
            hashHex.Should().Be(file.Sha256,
        ...
        // the original upload.
        var attachmentEntries = zip.Entries
            .Where(e => e.FullName.StartsWith("attachments/", StringComparison.Ordinal))
            .ToList();
        ...
        attachMs.ToArray().Should().Equal(attachmentBytes,
    ...
    public async Task DraftsInPeriod_Without_AllowDrafts_Throws()
    {
        ...
        var draft = PurchaseInvoice.CreateDraft(supplier.Id, supplier.TaxProfile,
            "SUP-INV-DRAFT", new DateOnly(2026, 6, 5));
        draft.AddLine(itemId: null, expenseCategoryId: Guid.NewGuid(),
        ...
        await act.Should().ThrowAsync<InvalidOperationException>()
            .Where(ex => ex.Message.Contains("draft", StringComparison.OrdinalIgnoreCase),
    ...
    public async Task DraftsInPeriod_With_AllowDrafts_RecordsExcludedIds_InManifest()
    {
        ...
        var draft = PurchaseInvoice.CreateDraft(supplier.Id, supplier.TaxProfile,
            "SUP-INV-DRAFT-2", new DateOnly(2026, 7, 8));
        draft.AddLine(itemId: null, expenseCategoryId: Guid.NewGuid(),
            quantity: 1m, unitPrice: MoneyEgp.From(50m),
            vatCategoryId: vat.Id, vatRatePercent: vat.RatePercent,
            deductibleFlag: false);
        ...
        result.Manifest.ExcludedDraftIds!.Should().Contain(draft.Id,
