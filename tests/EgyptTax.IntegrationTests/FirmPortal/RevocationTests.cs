...
public class RevocationTests(SqlServerFixture fixture)
{
    ...
    public async Task Revoke_FlipsActive_WritesAuditRow_PreservesPriorHistory()
    {
        ...
        var firmUser = await new InviteAccountantFirmUserHandler(
            db,
            inviteClock,
            auditStore
        ).InviteAsync(
        ...
        await new InviteAccountantFirmUserHandler(db, acceptClock, auditStore).AcceptAsync(
            new AcceptInvitationCommand(firmUser.UserId),
            CancellationToken.None
        );

        var auditEntriesBeforeRevoke = await db.Set<AuditLogEntry>()
            .AsNoTracking()
            .Where(e => e.ActorFirmName == "Nile Accounting LLC")
            .ToListAsync();
        auditEntriesBeforeRevoke
            .Should()
            .HaveCount(2, because: "two firm-tagged audit rows: invited + accepted");

        // Now revoke.
        var revokeClock = new TestClock(new DateTime(2026, 5, 8, 14, 0, 0, DateTimeKind.Utc));
        var revoked = await new InviteAccountantFirmUserHandler(
            db,
            revokeClock,
            auditStore
        ...
        var allAfter = await db.Set<AuditLogEntry>()
            .AsNoTracking()
            .Where(e => e.ActorFirmName == "Nile Accounting LLC")
            .OrderBy(e => e.Index)
            .ToListAsync();
        ...
        allAfter[2]
            .ActorUserId.Should()
            .Be(admin.Id, because: "the revoker is the actor on the revoke event");
    }
    ...
    public async Task Revoke_IsIdempotent_OriginalRevocationDataPreserved()
    {
        ...
        await new InviteAccountantFirmUserHandler(
            db,
            new TestClock(new DateTime(2026, 4, 2, 10, 0, 0, DateTimeKind.Utc)),
            auditStore
        ...
        var refreshed = await new InviteAccountantFirmUserHandler(
            db,
            new TestClock(new DateTime(2026, 5, 9, 14, 0, 0, DateTimeKind.Utc)),
            auditStore
        ).RevokeAsync(
            new RevokeFirmUserCommand(firmUser.UserId, admin2.Id),
            CancellationToken.None
        );

        refreshed
            .RevokedAtUtc.Should()
            .Be(firstRevoke, because: "idempotent — original revoked_at_utc wins");
        refreshed
            .RevokedByUserId.Should()
            .Be(admin.Id, because: "idempotent — original revoker wins");

        db.ChangeTracker.Clear();
        var revokeRows = await db.Set<AuditLogEntry>()
            .AsNoTracking()
            .Where(e => e.Kind == "firm_user.revoked")
            .ToListAsync();
        revokeRows
            .Should()
            .HaveCount(1, because: "the second revoke MUST NOT emit a duplicate audit row");
    }
