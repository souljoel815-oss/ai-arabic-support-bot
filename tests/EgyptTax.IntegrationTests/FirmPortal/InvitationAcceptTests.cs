...
public class InvitationAcceptTests(SqlServerFixture fixture)
{
    ...
    public async Task Invite_ThenAccept_TagsAuditLogWithFirmName()
    {
        ...
        var invite = await handler.InviteAsync(new InviteAccountantFirmUserCommand(
        ...
        invite.AcceptedAtUtc.Should().BeNull(because: "invitation just issued; awaiting accept handshake");
        ...
        var accepted = await handler2.AcceptAsync(new AcceptInvitationCommand(invite.UserId), CancellationToken.None);

        accepted.AcceptedAtUtc.Should().Be(clock2.UtcNow,
            because: "Accept stamps the row's accepted_at_utc to the current clock");
        accepted.IsActive.Should().BeTrue(
            because: "FR-049 — accepted AND not revoked == active firm-user");
        ...
        entries[0].ActorFirmName.Should().Be("Nile Accounting LLC",
            because: "every firm-user-related audit row carries the firm name for inspector traceability");
        entries[0].ActorUserId.Should().Be(admin.Id,
            because: "the inviter is the actor on the invite event");
        entries[1].Kind.Should().Be("firm_user.accepted");
        entries[1].ActorFirmName.Should().Be("Nile Accounting LLC");
        entries[1].ActorUserId.Should().Be(invite.UserId,
            because: "the invitee is the actor on the accept event");
    }
    ...
    public async Task Accept_IsIdempotent_OriginalTimestampWins()
    {
        ...
        var auditStore = new SqlAuditLogStore(db);
        var inviteHandler = new InviteAccountantFirmUserHandler(db,
            new TestClock(new DateTime(2026, 5, 8, 9, 0, 0, DateTimeKind.Utc)), auditStore);
        var firmUser = await inviteHandler.InviteAsync(new InviteAccountantFirmUserCommand(
            InviteeEmail: $"acc-{Guid.NewGuid():N}@nileaccounting.eg",
            InviteeDisplayName: new ArabicEnglishText("محاسب", "Accountant"),
            FirmName: "Nile Accounting LLC",
            FirmExternalIdentifier: "nileaccounting.eg",
        ...
        // First accept.
        await new InviteAccountantFirmUserHandler(db, new TestClock(firstAccept), auditStore)
        ...
        var refreshed = await new InviteAccountantFirmUserHandler(db, secondClock, auditStore)
            .AcceptAsync(new AcceptInvitationCommand(firmUser.UserId), CancellationToken.None);

        refreshed.AcceptedAtUtc.Should().Be(firstAccept,
            because: "idempotent — original accept timestamp wins");

        db.ChangeTracker.Clear();
        var acceptedEntries = await db.Set<AuditLogEntry>().AsNoTracking()
            .Where(e => e.Kind == "firm_user.accepted")
            .ToListAsync();
        acceptedEntries.Should().HaveCount(1,
            because: "the second accept call MUST NOT emit a duplicate audit row");
    }
