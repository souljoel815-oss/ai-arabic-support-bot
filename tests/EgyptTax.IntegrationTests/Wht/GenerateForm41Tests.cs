...
public class GenerateForm41Tests(SqlServerFixture fixture)
{
    ...
    public async Task Generate_AggregatesQuarterlyCertificates_IntoSchemaShapedPayload()
    {
        ...
        result
            ...
            .Be(750m, because: "10k × 5% + 2.5k × 10% = 500 + 250 = 750");
        ...
        result
            .Payload.Totals.ByCategory.Single(c => c.WhtCategoryCode == "Services")
            .AmountWithheld.Should()
            .Be(500m);
        result
            .Payload.Totals.ByCategory.Single(c => c.WhtCategoryCode == "Professional")
            .AmountWithheld.Should()
            .Be(250m);

        // Reconciliation: WHT-payable accrual matches the cert total.
        result
            .Payload.Reconciliation.WhtPayableAccountBalanceAtPeriodEnd.Should()
            .Be(
                750m,
    ...
    public async Task Generate_RefusesDuplicateForSameQuarter()
    {
        ...
        await handler.HandleAsync(
            new GenerateForm41Command(2026, 1, user.Id),
            CancellationToken.None
        );
        ...
        await act.Should()
            ...
            .Where(ex =>
                ex.Message.Contains("already been generated", StringComparison.OrdinalIgnoreCase)
            );
    }
    ...
    public async Task Generate_EmptyQuarter_ProducesValidPayloadWithZeroLines()
    {
        ...
        result
            ...
            .BeTrue(because: "0 = 0; an empty quarter reconciles by definition");
