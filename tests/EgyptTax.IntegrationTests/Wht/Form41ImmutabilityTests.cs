...
public class Form41ImmutabilityTests(SqlServerFixture fixture)
{
    ...
    public async Task MarkFiled_StampsAllCertsInQuarter_AndRefusesSecondCall()
    {
        ...
        var filed = await mark.HandleAsync(
            new MarkForm41FiledCommand(generated.Form41FilingId, user.Id),
            CancellationToken.None
        );
        ...
        var stamped = await db.Set<WhtCertificate>()
            .AsNoTracking()
    ...
    public async Task RegenerateAfterFile_ProducesEmptyPayload_BecauseAllCertsStamped()
    {
        ...
        var filing = await db.Set<Form41Filing>().FirstAsync(f => f.Id == generated.Form41FilingId);
        db.Remove(filing);
    ...
    public async Task MarkFiled_RefusedWhenReconciliationIsDirty()
    {
        ...
        var manualHandler =
            new EgyptTax.Infrastructure.Journals.CreateManualAdjustingJournalHandler(
                db,
                new TestClock(new DateTime(2026, 5, 11, 10, 0, 0, DateTimeKind.Utc)),
        ...
        await manualHandler.HandleAsync(
            new EgyptTax.Application.Journals.CreateManualAdjustingJournalCommand(
                ...
                Narration: new ArabicEnglishText(
                    "غموض",
                    "Manual adjust that breaks reconciliation"
                ),
        ...
        db.ChangeTracker.Clear();
        var refreshed = await db.Set<Form41Filing>()
            .AsNoTracking()
    ...
    public void Cert_MarkIncludedInForm41Filing_RefusesSecondStamp_AtAggregateBoundary()
    {
        ...
        act.Should()
            ...
            .Where(ex =>
                ex.Message.Contains("already included", StringComparison.OrdinalIgnoreCase)
            );
    }
