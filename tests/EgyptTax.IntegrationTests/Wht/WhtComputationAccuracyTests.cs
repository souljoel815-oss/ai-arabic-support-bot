...
public class WhtComputationAccuracyTests(SqlServerFixture fixture)
{
    ...
    public async Task SC011_FiftyPaymentsAtVaryingRates_FormFortyOneReconciles_AndDashboardMatches()
    {
        ...
        var royalties20 = new WhtCategory(
            ...
            effectiveToDate: null,
            applicableTo: WhtApplicableTo.SuppliersServices);
        db.Add(services5); db.Add(professional10); db.Add(royalties20);
        ...
        {
            ["Services"] = 0m, ["Professional"] = 0m, ["Royalties"] = 0m,
        };
        var perCategoryCount = new Dictionary<string, int>
        {
            ["Services"] = 0, ["Professional"] = 0, ["Royalties"] = 0,
        };
        ...
        for (var i = 0; i < InvoicesPerQuarter; i++)
        {
            ...
            var gross = decimal.Round(500m + (decimal)(rng.NextDouble() * 24_500), 2, MidpointRounding.ToEven);
            ...
            if (i % 10 == 9) db.ChangeTracker.Clear();
        }
        ...
        result.Payload.Lines.Should().HaveCount(InvoicesPerQuarter,
        ...
        certNumbers.Distinct().Count().Should().Be(InvoicesPerQuarter,
        ...
        result.Payload.Totals.TotalAmountWithheld.Should().Be(totalWithheld,
        ...
        {
            var rollUp = result.Payload.Totals.ByCategory.SingleOrDefault(c => c.WhtCategoryCode == category);
        ...
        var dashboard = await new SqlWhtLifecycleDashboardQuery(db)
            .GetAsync(asOf: new DateOnly(2026, 7, 5));
        dashboard.Owed.CertCount.Should().Be(InvoicesPerQuarter);
        dashboard.Owed.TotalAccruedNotYetFiled.Should().Be(totalWithheld,
