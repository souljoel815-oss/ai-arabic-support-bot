...
public class CustomerReceiptWhtSplitTests(SqlServerFixture fixture)
{
    ...
    public async Task CustomerReceipt_WithCustomerWhtCertificate_Emits3LineJe_AndRecordsInboundCertificate()
    {
        ...
        var postedSales = await new PostSalesInvoiceHandler(
            db,
        ...
        ).HandleAsync(new PostSalesInvoiceCommand(draft.Id, user.Id), CancellationToken.None);
        ...
        var posted = await new PostCustomerReceiptVoucherHandler(
            db,
            new SqlSequentialNumberAllocator(db),
        ...
        // Voucher state.
        posted
            .WhtReceivableAmount.Amount.Should()
            .Be(57m, because: "customer's number is authoritative — 57 withheld");
        posted
            .NetCashReceived.Amount.Should()
            .Be(1_083m, because: "1140 gross − 57 customer-withheld = 1083 cash received");
