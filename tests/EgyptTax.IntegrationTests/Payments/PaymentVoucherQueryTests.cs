...
public class PaymentVoucherQueryTests(SqlServerFixture fixture)
{
    ...
    public async Task ListOutstandingSalesInvoices_ShowsOnlyInvoicesWithOpenBalance()
    {
        ...
        var vA = CustomerReceiptVoucher.CreateDraft(
            customer.Id,
        ...
        var vB = CustomerReceiptVoucher.CreateDraft(
            customer.Id,
        ...
        outstanding
            .Should()
            .HaveCount(1, because: "the fully-settled invoice MUST drop off the outstanding list");
        outstanding[0].InvoiceId.Should().Be(partial.Id);
        outstanding[0].AllocatedToDate.Amount.Should().Be(500m);
        outstanding[0]
            .OpenBalance.Amount.Should()
            .Be(640m, because: "1140 grand total − 500 allocated = 640 open balance");
    }
    ...
    public async Task GetInvoiceAllocations_RollsUpMultiVoucherHistory_AndExposesOpenBalance()
    {
        ...
        var view = await new SqlPaymentVoucherQuery(db).GetInvoiceAllocationsAsync(
            posted.Id,
        ...
        view.OpenBalance.Amount.Should()
            .Be(140m, because: "1140 grand total − 1000 allocated = 140 open balance");
