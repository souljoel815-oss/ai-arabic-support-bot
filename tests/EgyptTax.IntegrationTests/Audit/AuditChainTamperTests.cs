...
public class AuditChainTamperTests(SqlServerFixture fixture)
{
    ...
    private static AuditLogPayload Payload(int i) => new(
        Kind: "Synthetic",
    ...
    public async Task Case2_InsertAtMid_ReportsPrevHashMismatch()
    {
        ...
        var tampered = await db.Set<AuditLogEntry>().AsNoTracking().OrderBy(e => e.Index).ToListAsync();
    ...
    public async Task Case3_EditAtMid_ReportsThisHashMismatch()
    {
        ...
        report.Findings.Should().Contain(f =>
            f.Kind == AuditChainFindingKind.ThisHashMismatch && f.AtIndex == 3);
    }
    ...
    public async Task Case4_DeleteAtMid_ReportsMissingIndex()
    {
        ...
        report.Findings.Should().Contain(f =>
            f.Kind == AuditChainFindingKind.MissingIndex && f.AtIndex == 3);
    }
    ...
    public async Task Case5_ReorderTwoEntries_ReportsPrevHashMismatch()
    {
        ...
        await db.Database.ExecuteSqlRawAsync("UPDATE [audit].[audit_log] SET [index] = -2 WHERE [index] = 2");
        await db.Database.ExecuteSqlRawAsync("UPDATE [audit].[audit_log] SET [index] = 2 WHERE [index] = 3");
        await db.Database.ExecuteSqlRawAsync("UPDATE [audit].[audit_log] SET [index] = 3 WHERE [index] = -2");
    ...
    public async Task Case7_CheckpointStaleButTailIntact_ReportsCheckpointMismatch()
    {
        ...
        report.IsValid.Should().BeFalse();
        report.Findings.Should().Contain(f =>
            f.Kind == AuditChainFindingKind.CheckpointMismatch && f.AtIndex == 5);
    }
    ...
    public async Task Case8_CheckpointTamperedOnly_ReportsCheckpointMismatch()
    {
        ...
        forgedHash[0] = 0xDE;
        forgedHash[1] = 0xAD;
        var forgedCheckpoint = new AuditCheckpoint(LastIndex: 5, LastHash: forgedHash, TsUtc: DateTime.UtcNow);
