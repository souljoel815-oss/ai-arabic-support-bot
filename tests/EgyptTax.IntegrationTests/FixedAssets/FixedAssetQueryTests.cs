...
public class FixedAssetQueryTests(SqlServerFixture fixture)
{
    ...
    public async Task ListAsync_ReturnsRows_WithComputedNbv_AndAttachmentCount()
    {
        ...
        var assetA = FixedAsset.CreateDraft(
            code: "FA-LIST-A",
            description: new ArabicEnglishText("معدات", "List asset A"),
        ...
        var assetB = FixedAsset.CreateDraft(
            code: "FA-LIST-B",
            description: new ArabicEnglishText("مسودة", "List asset B (draft)"),
            assetCategory: "Equipment",
            cost: MoneyEgp.From(12_000m),
            inServiceDate: new DateOnly(2026, 6, 1),
            usefulLifeMonths: 60,
            depreciationMethod: DepreciationMethod.StraightLine,
            salvageValue: MoneyEgp.Zero,
            convention: DepreciationConvention.FullMonth
        ...
        rowA.AttachmentCount.Should().Be(2, because: "two attachments uploaded for asset A");
        rowA.NetBookValue.Amount.Should()
            .Be(
                57_000m,
        ...
        rowB.NetBookValue.Amount.Should()
            .Be(12_000m, because: "Draft assets aren't depreciable yet — NBV stays at cost");
    }
    ...
    public async Task CreateFixedAsset_PersistsDraft_AndEmitsCreatedAuditEvent()
    {
        ...
        var handler = new CreateFixedAssetHandler(
            db,
            new TestClock(new DateTime(2026, 5, 9, 11, 0, 0, DateTimeKind.Utc)),
            audit
        );

        var asset = await handler.HandleAsync(
        ...
        var reloaded = await db.Set<FixedAsset>().AsNoTracking().FirstAsync(a => a.Id == asset.Id);
    ...
    public async Task CreateFixedAsset_DuplicateCode_IsRejected()
    {
        ...
        var count = await db.Set<FixedAsset>().AsNoTracking().CountAsync(a => a.Code == "FA-DUP");
        count
            .Should()
            .Be(1, because: "exactly one FA-DUP row exists; the rejection prevented the second");
    }
