...
public class AttachmentIntegrityTests(SqlServerFixture fixture) : IDisposable
{
    ...
    private readonly string _tempRoot = Path.Combine(
        Path.GetTempPath(), $"egypttax-attach-{Guid.NewGuid():N}");
    ...
    public async Task Upload_RoundTrips_Sha256_AndContent_Bytes()
    {
        ...
        var attachment = await handler.HandleAsync(new UploadAttachmentCommand(
        ...
        attachment.Sha256.Should().Equal(expectedSha256,
        ...
        attachment.RelativePath.Should().StartWith("attachments/",
        ...
        sink.ToArray().Should().Equal(originalBytes,
        ...
        diskHash.Should().Equal(attachment.Sha256,
    ...
    public async Task BitRot_Is_Detectable_By_Rehashing_The_OnDiskFile()
    {
        ...
        await using var contentStream = new MemoryStream(originalBytes);
        var attachment = await handler.HandleAsync(new UploadAttachmentCommand(
            parent.Id, DocumentType.PurchaseInvoice,
            "receipt.pdf", "application/pdf",
            contentStream, user.Id), CancellationToken.None);

        // Simulate bit-rot: tamper the on-disk file directly.
        var absolutePath = Path.Combine(
            _tempRoot, attachment.RelativePath.Replace('/', Path.DirectorySeparatorChar));
        ...
        diskHash.Should().NotEqual(attachment.Sha256,
    ...
    public async Task Upload_Rejects_Disallowed_Mime_Type()
    {
        ...
        var act = async () => await handler.HandleAsync(new UploadAttachmentCommand(
            parent.Id, DocumentType.PurchaseInvoice,
    ...
    private static async Task<(PurchaseInvoice, EgyptTax.Domain.Identity.User)> SeedParentAsync(AppDbContext db)
    {
        ...
        db.Add(vat); db.Add(supplier); db.Add(user); db.Add(draft);
