...
public class ClosingCockpitAggregationTests(SqlServerFixture fixture)
{
    ...
    public async Task Cockpit_Aggregates_VatReadiness_Drafts_FailedEta_MissingAttachments_PeriodLockChecklist()
    {
        ...
        await PostSalesAsync(db, customer, item, vat, user,
            new DateOnly(2026, 5, 5), 1_000m, etaFailureRate: 0.0);

        // Posted sales invoice with FAILED ETA submission (100% mock).
        await PostSalesAsync(db, customer, item, vat, user,
            new DateOnly(2026, 5, 8), 2_000m, etaFailureRate: 1.0);

        // Posted purchase with deductible line + attachment (clean).
        await PostPurchaseAsync(db, supplier, vat, user,
            new DateOnly(2026, 5, 10), 800m, deductible: true, attachments: 1, "SUP-001");
        ...
        await PostExpenseDirectlyAsync(db, category, user,
            new DateOnly(2026, 5, 12), 300m, deductible: true, attachmentCount: 0);

        // Draft sales invoice in period (must clear before lock).
        var draftSales = SalesInvoice.CreateDraft(
            customer.Id, customer.TaxProfile, new DateOnly(2026, 5, 20));
        ...
        cockpit.TotalPostsInPeriod.Should().BeGreaterThanOrEqualTo(4,
            because: "2 sales + 1 purchase + 1 expense = 4 posted documents minimum");
        cockpit.DraftsInPeriodCount.Should().BeGreaterThanOrEqualTo(1,
            because: "the May draft sales invoice MUST surface in the drafts count");

        cockpit.FailedEtaSubmissionCount.Should().BeGreaterThanOrEqualTo(1,
            because: "one sales invoice was posted with the 100%-failure mock submitter");

        cockpit.MissingDocuments.Should().Contain(b => b.Name.Contains("deductible expenses with no attachment", StringComparison.OrdinalIgnoreCase),
            because: "the deductible-expense-with-no-attachment bucket MUST be surfaced");

        cockpit.VatReadinessPercent.Should().BeLessThan(100m,
    ...
    public async Task Cockpit_Reflects_LockedPeriod_State_FromTaxPeriodTable()
    {
        ...
        await new LockTaxPeriodHandler(db, lockClock, new CaptureAuditLogStore())
            .HandleAsync(new LockTaxPeriodCommand(
                EgyptTax.Domain.Periods.TaxPeriodKind.VatMonth,
                2026, 6, admin.Id, "filed"), CancellationToken.None);

        var cockpit = await new SqlMonthlyTaxClosingCockpitQuery(db).RunAsync(2026, 6);

        cockpit.PeriodIsLocked.Should().BeTrue();
        cockpit.PeriodLockChecklist.Single(c => c.Description.Contains("not yet locked", StringComparison.OrdinalIgnoreCase))
    ...
    public async Task EmptyPeriod_Yields_100Percent_Readiness_AndCleanChecklist()
    {
        ...
        cockpit.VatReadinessPercent.Should().Be(100m,
    ...
    private static async Task PostSalesAsync(AppDbContext db, Customer customer, Item item,
        VatCategory vat, User user, DateOnly date, decimal unitPrice, double etaFailureRate)
    {
        ...
        var inner = new PostSalesInvoiceHandler(db,
        ...
        var wrapper = new PostSalesInvoiceWithEtaSubmissionHandler(
            inner, db, new MockEtaSubmitter(etaFailureRate),
            new EInvoiceJsonGenerator(), new CaptureAuditLogStore(), clock);
        await wrapper.HandleAsync(new PostSalesInvoiceCommand(draft.Id, user.Id), CancellationToken.None);
    }

    private static async Task PostPurchaseAsync(AppDbContext db, Supplier supplier, VatCategory vat,
        User user, DateOnly date, decimal unitPrice, bool deductible, int attachments,
    ...
    /// </summary>
    private static async Task PostExpenseDirectlyAsync(AppDbContext db, DeductibleExpenseCategory category,
        User user, DateOnly date, decimal amount, bool deductible, int attachmentCount)
    {
        var expense = Expense.CreateDraft(date, category.Id, MoneyEgp.From(amount),
            deductible, new ArabicEnglishText("وصف", "Test expense"));
        expense.MarkPosted(
        ...
        {
            db.Add(new Attachment(expense.Id, DocumentType.Expense,
                $"r-{i}.pdf", $"{Guid.NewGuid():N}.pdf",
