...
public class TrialBalanceReportTests(SqlServerFixture fixture)
{
    ...
    public async Task SinglePostedSale_Yields_BalancedTrialBalance()
    {
        ...
        var handler = new PostSalesInvoiceHandler(db,
            new SqlSequentialNumberAllocator(db), clock, new CaptureAuditLogStore(), emitter);
        ...
        report.Rows.Should().Contain(r => r.AccountCode == ChartOfAccountCodes.AccountsReceivable
            && r.TotalDebit.Amount == 1_140m && r.TotalCredit.Amount == 0m);
        report.Rows.Should().Contain(r => r.AccountCode == ChartOfAccountCodes.SalesRevenue
            && r.TotalCredit.Amount == 1_000m && r.TotalDebit.Amount == 0m);
        report.Rows.Should().Contain(r => r.AccountCode == ChartOfAccountCodes.OutputVatPayable
            && r.TotalCredit.Amount == 140m && r.TotalDebit.Amount == 0m);
    }
    ...
    public async Task DocumentsOutsidePeriod_Are_NotIncluded_InTrialBalance()
    {
        ...
        var draft = SalesInvoice.CreateDraft(customer.Id, customer.TaxProfile, new DateOnly(2026, 4, 30));
        ...
        mayReport.IsBalanced.Should().BeTrue(
            because: "0 == 0 is balanced — the empty case is balanced too");
    }

    private static async Task<(Customer customer, Item item, VatCategory vat, User user)>
