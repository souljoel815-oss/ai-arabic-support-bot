    ...
    public async Task Sc005_TwoHundredSalesPlusTwoHundredPurchases_MatchesManualControl_WithinOneEgp()
    {
        ...
        for (var i = 0; i < 200; i++)
        {
            ...
            var draft = SalesInvoice.CreateDraft(
                fixtureData.Customer.Id,
                fixtureData.Customer.TaxProfile,
                date
            );
            draft.AddLine(
                fixtureData.Item.Id,
                ...
                fixtureData.Vat.Id,
                fixtureData.Vat.RatePercent
            );
            db.Add(draft);
            await db.SaveChangesAsync();

            var clock = new TestClock(date.ToDateTime(new TimeOnly(11, 0)).ToUniversalTime());
            await new PostSalesInvoiceHandler(
                db,
                allocator,
                clock,
                new NoOpAuditLogStore()
            ).HandleAsync(
                new PostSalesInvoiceCommand(draft.Id, fixtureData.User.Id),
        ...
        for (var i = 0; i < 200; i++)
        {
            ...
            var draft = PurchaseInvoice.CreateDraft(
                fixtureData.Supplier.Id,
                fixtureData.Supplier.TaxProfile,
                $"SUP-{i.ToString(CultureInfo.InvariantCulture)}-{Guid.NewGuid():N}".Substring(
                    0,
                    24
                ),
                date
            ...
            draft.AddLine(
                ...
                quantity: 1m,
                unitPrice: MoneyEgp.From(amount),
                vatCategoryId: fixtureData.Vat.Id,
                vatRatePercent: fixtureData.Vat.RatePercent,
            ...
            var clock = new TestClock(date.ToDateTime(new TimeOnly(11, 0)).ToUniversalTime());
            await new PostPurchaseInvoiceHandler(
                db,
                allocator,
                clock,
                new NoOpAuditLogStore()
            ).HandleAsync(
                new PostPurchaseInvoiceCommand(draft.Id, fixtureData.User.Id),
                CancellationToken.None
            );
        }
        ...
        report
            .DeductibleExpenses.Amount.Should()
            .BeApproximately(
                controlDeductibleExpenses,
                1m,
                because: "SC-005 — DeductibleExpenses MUST match within ±1 EGP at 100 deductible-purchase scale"
            );
        report
            .NonDeductibleAdjustments.Amount.Should()
            .BeApproximately(
                controlNonDeductibleAdjustments,
                1m,
        ...
        report
            ...
            .BeApproximately(
                controlRevenue - controlDeductibleExpenses,
                1m,
