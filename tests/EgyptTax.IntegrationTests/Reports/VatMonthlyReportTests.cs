...
public class VatMonthlyReportTests(SqlServerFixture fixture)
{
    ...
    public async Task US2_Scenario1_Sales10k_Plus_Purchase4k_NetPayable_Is_840()
    {
        ...
        await PostSalesInvoiceAsync(db, customer, item, vat, user,
            documentDate: new DateOnly(2026, 5, 15), unitPriceEgp: 10_000m);

        // Deductible purchase: 4,000 net + 560 VAT against a
        // RegisteredTaxpayer supplier (so VAT is recoverable).
        await PostPurchaseInvoiceAsync(db, supplier, vat, user,
            dateReceived: new DateOnly(2026, 5, 20), unitPriceEgp: 4_000m,
            deductible: true, supplierInvoiceNumber: "SUP-001");

        var query = new SqlVatMonthlyReportQuery(db);
        var report = await query.RunAsync(2026, 5);

        report.OutputVat.Amount.Should().Be(1_400m,
            because: "10,000 EGP at 14% = 1,400 EGP output VAT");
        report.InputVatRecoverable.Amount.Should().Be(560m,
            because: "4,000 EGP at 14% = 560 EGP input VAT (supplier is RegisteredTaxpayer so it's recoverable)");
        report.NetPayable.Amount.Should().Be(840m,
            because: "1,400 - 560 = 840 EGP owed to ETA per US2 acceptance scenario 1");
    ...
    public async Task PurchaseFrom_UnregisteredSupplier_DoesNot_Contribute_To_InputVat()
    {
        ...
        await PostSalesInvoiceAsync(db, customer, item, vat, user,
            documentDate: new DateOnly(2026, 6, 10), unitPriceEgp: 10_000m);
        await PostPurchaseInvoiceAsync(db, unregistered, vat, user,
            dateReceived: new DateOnly(2026, 6, 12), unitPriceEgp: 5_000m,
            deductible: true, supplierInvoiceNumber: "UNREG-001");

        var report = await new SqlVatMonthlyReportQuery(db).RunAsync(2026, 6);

        report.OutputVat.Amount.Should().Be(1_400m);
        report.InputVatRecoverable.Amount.Should().Be(0m,
    ...
    public async Task NonDeductible_PurchaseLines_DoNot_Contribute_To_InputVat()
    {
        ...
        await PostSalesInvoiceAsync(db, customer, item, vat, user,
            documentDate: new DateOnly(2026, 7, 1), unitPriceEgp: 1_000m);
        await PostPurchaseInvoiceAsync(db, supplier, vat, user,
            dateReceived: new DateOnly(2026, 7, 2), unitPriceEgp: 500m,
            deductible: false, supplierInvoiceNumber: "SUP-NONDED");
    ...
    public async Task DocumentsOutsidePeriod_Are_NotIncluded()
    {
        ...
        await PostSalesInvoiceAsync(db, customer, item, vat, user,
            documentDate: new DateOnly(2026, 4, 30), unitPriceEgp: 1_000m);
    ...
    private static async Task PostSalesInvoiceAsync(
        AppDbContext db, Customer customer, Item item, VatCategory vat,
        User user, DateOnly documentDate, decimal unitPriceEgp)
    {
    ...
    private static async Task PostPurchaseInvoiceAsync(
        AppDbContext db, Supplier supplier, VatCategory vat, User user,
        DateOnly dateReceived, decimal unitPriceEgp, bool deductible,
        string supplierInvoiceNumber)
    {
        var draft = PurchaseInvoice.CreateDraft(
            supplier.Id, supplier.TaxProfile, supplierInvoiceNumber, dateReceived);
        draft.AddLine(itemId: null, expenseCategoryId: Guid.NewGuid(),
            quantity: 1m, unitPrice: MoneyEgp.From(unitPriceEgp),
    ...
    private static async Task<(Customer customer, Supplier supplier, Item item, VatCategory vat, User user)>
