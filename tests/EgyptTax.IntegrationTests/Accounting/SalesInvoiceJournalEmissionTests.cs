...
public class SalesInvoiceJournalEmissionTests(SqlServerFixture fixture)
{
    ...
    public async Task PostSalesInvoice_Emits_ThreeBalancedJournalLines()
    {
        ...
        vat.Credit.Amount.Should().Be(140m, because: "Output VAT Payable is credited with the VAT total");
    ...
    public async Task PostCreditNote_Emits_ReversedJournalLines()
    {
        ...
        var draft = await issueHandler.HandleAsync(new IssueCreditNoteCommand(
            originalInvoice.Id,
            new[] { new IssueCreditNoteLine(dependencies.Item.Id, -1m, MoneyEgp.From(1_000m),
                dependencies.Vat.Id, dependencies.Vat.RatePercent) },
        ...
        var sumCredits = entry.Lines.Sum(l => l.Credit.Amount);
        sumDebits.Should().Be(sumCredits,
            because: "the credit-note journal MUST also be balanced");
    }
    ...
    public async Task JournalEntry_AndOriginalInvoice_CommitInTheSameTransactionAsThePost()
    {
        ...
        var hasJournal = await db.Set<JournalEntry>().AnyAsync(e => e.SourceDocumentId == invoice.Id);
        hasJournal.Should().BeTrue(because: "the journal entry persists alongside the post");

        var posted = await db.Set<SalesInvoice>().AsNoTracking().FirstAsync(i => i.Id == invoice.Id);
    ...
    public async Task TwoLineMixedRate_StillProducesBalancedJournal()
    {
        ...
        var entry = await db.Set<JournalEntry>()
            .Include(e => e.Lines).AsNoTracking()
        ...
        entry.Lines.Single(l => l.AccountCode == ChartOfAccountCodes.OutputVatPayable)
            .Credit.Amount.Should().Be(84m);
    }

    private static async Task<(SalesInvoice posted, FixtureDeps deps)>
        PostInvoiceWithJournalAsync(AppDbContext db)
    {
    ...
    private sealed record FixtureDeps(Customer Customer, Item Item, VatCategory Vat, User OperatorUser);
    ...
    private static async Task<(Customer, Item, VatCategory, User)> SeedAsync(AppDbContext db)
    {
        ...
        db.Add(vat); db.Add(customer); db.Add(item); db.Add(company); db.Add(user);
        await db.SaveChangesAsync();
