...
public class JournalBalanceStressTests(SqlServerFixture fixture)
{
    ...
    public async Task OneThousandRandomizedPostedDocuments_AllJournals_BalanceToTheCent()
    {
        ...
        var purchaseHandler = new PostPurchaseInvoiceHandler(db, allocator, clock, audit, purchaseEmitter);
        ...
        for (var i = 0; i < DocumentCount; i++)
        {
            ...
            var unitPrice = MoneyEgp.From(decimal.Round(100m + (decimal)(rng.NextDouble() * 9_900), 2, MidpointRounding.ToEven));
            ...
            else
            {
                var draft = PurchaseInvoice.CreateDraft(supplier.Id, supplier.TaxProfile,
                    $"SUP-T166-{i:D4}", new DateOnly(2026, 5, 9));
                draft.AddLine(itemId: null, expenseCategoryId: Guid.NewGuid(),
                    quantity, unitPrice, vat.Id, vat.RatePercent,
                    deductibleFlag: false); // non-deductible — no FR-016 attachment needed
                db.Add(draft);
                await db.SaveChangesAsync();
    ...
    private static async Task<(Customer, Supplier, VatCategory, Item, User)> SeedAsync(AppDbContext db)
    {
