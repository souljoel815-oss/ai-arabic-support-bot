...
public class JournalLedgerQueryTests(SqlServerFixture fixture)
{
    ...
    public async Task ListAsync_Unions_Auto_Manual_AndReversal_InOnePeriodStream()
    {
        ...
        var manual = await new CreateManualAdjustingJournalHandler(
            db,
            clock,
            new CaptureAuditLogStore()
        ).HandleAsync(
            new CreateManualAdjustingJournalCommand(
        ...
        var reversal = await new CreateReversalJournalHandler(
            db,
            clock,
            new CaptureAuditLogStore()
        ).HandleAsync(
            new CreateReversalJournalCommand(
        ...
        rows.Should()
            .Contain(
                r => r.Kind == JournalEntryKind.AutoEmitted,
    ...
    public async Task GetAsync_AutoEmitted_SurfacesSourceDocumentForFr025DrillBack()
    {
        ...
        db.Add(draft);
        await db.SaveChangesAsync();
        var posted = await new PostSalesInvoiceHandler(
            db,
            new SqlSequentialNumberAllocator(db),
            clock,
            new CaptureAuditLogStore(),
        ...
        var entry = await db.Set<EgyptTax.Domain.Accounting.JournalEntry>()
            .AsNoTracking()
            .FirstAsync(e => e.SourceDocumentId == posted.Id);

        var detail = await new SqlJournalLedgerQuery(db).GetAsync(
            entry.Id,
    ...
    public async Task GetAsync_Reversal_SurfacesReversesJournalVoucherIdForFr025DrillBack()
    {
        ...
        detail
            .ReversesJournalVoucherId.Should()
            .Be(
                manual.Id,
    ...
    private static async Task<(Customer, Item, VatCategory, User)> SeedAsync(AppDbContext db)
    {
        ...
        var accountantRole = new Role(
            code: "Accountant",
            name: new ArabicEnglishText("محاسب", "Accountant"),
            requiresMfa: false
        );
        var user = new User(
