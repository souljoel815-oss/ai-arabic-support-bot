...
public class DocumentApprovalTests(SqlServerFixture fixture)
{
    ...
    public async Task US3_HappyPath_Draft_Submitted_Approved()
    {
        ...
        var request = await handler.SubmitAsync(new SubmitDocumentCommand(
            draft.Id, DocumentType.SalesInvoice, bookkeeper.Id), CancellationToken.None);

        var afterSubmit = await db.Set<SalesInvoice>().AsNoTracking().FirstAsync(i => i.Id == draft.Id);
        afterSubmit.State.Should().Be(DocumentState.Submitted,
            because: "Submit transition MUST move Draft → Submitted per FR-026");

        // Step 2: Approver approves.
        await handler.ApproveAsync(new ApproveDocumentCommand(
            draft.Id, DocumentType.SalesInvoice, approver.Id), CancellationToken.None);

        var afterApprove = await db.Set<SalesInvoice>().AsNoTracking().FirstAsync(i => i.Id == draft.Id);
        afterApprove.State.Should().Be(DocumentState.Approved);

        var freshRequest = await db.Set<ApprovalRequest>().AsNoTracking().FirstAsync(r => r.Id == request.Id);
    ...
    public async Task US3_Rejection_Returns_To_Draft_With_Reason_Captured()
    {
        ...
        await handler.SubmitAsync(new SubmitDocumentCommand(
            draft.Id, DocumentType.SalesInvoice, bookkeeper.Id), CancellationToken.None);
        await handler.RejectAsync(new RejectDocumentCommand(
            draft.Id, DocumentType.SalesInvoice, approver.Id,
            "VAT category looks wrong — should be Standard, not ZeroRated."),
            CancellationToken.None);

        var afterReject = await db.Set<SalesInvoice>().AsNoTracking().FirstAsync(i => i.Id == draft.Id);
        afterReject.State.Should().Be(DocumentState.Draft,
            because: "Reject MUST return state to Draft so the bookkeeper can edit + resubmit");

        var request = await db.Set<ApprovalRequest>().AsNoTracking().FirstAsync(r => r.DocumentId == draft.Id);
        ...
        audit.Captured.Single(e => e.Kind == "document.rejected")
            .PayloadJson.Should().Contain("VAT category looks wrong",
    ...
    public async Task FR004_Self_Approval_Is_Forbidden()
    {
        ...
        var handler = new DocumentApprovalHandler(db,
        ...
        var act = async () => await handler.ApproveAsync(new ApproveDocumentCommand(
            draft.Id, DocumentType.SalesInvoice, bookkeeper.Id), CancellationToken.None);
        ...
        var afterFailedApprove = await db.Set<SalesInvoice>().AsNoTracking().FirstAsync(i => i.Id == draft.Id);
        afterFailedApprove.State.Should().Be(DocumentState.Submitted,
    ...
    public async Task Void_OnDraft_Succeeds_OnPosted_IsBlocked()
    {
        ...
        await handler.VoidAsync(new VoidDocumentCommand(
            draft.Id, DocumentType.SalesInvoice, bookkeeper.Id, "Test void"),
            CancellationToken.None);
        ...
        var direct = SalesInvoice.CreateDraft(customer.Id, customer.TaxProfile, new DateOnly(2026, 5, 8));
        direct.AddLine(item.Id, 1m, MoneyEgp.From(200m), vat.Id, vat.RatePercent);
        direct.MarkPosted("INV-2026-TEST-1", bookkeeper.Id,
            new DateTime(2026, 5, 8, 11, 0, 0, DateTimeKind.Utc),
            DocumentPostingMode.UnapprovedDirect, approvalEnabled: false);
        db.Add(direct);
        await db.SaveChangesAsync();

        var voidPosted = async () => await handler.VoidAsync(new VoidDocumentCommand(
            direct.Id, DocumentType.SalesInvoice, bookkeeper.Id, "should fail"),
            CancellationToken.None);

        await voidPosted.Should().ThrowAsync<InvalidOperationException>()
            .Where(ex => ex.Message.Contains("Posted", StringComparison.OrdinalIgnoreCase),
                because: "FR-027 — Posted is terminal; corrections route through credit notes (FR-013) or reversal vouchers");
    }

    private static async Task<(Customer, Item, VatCategory, User bookkeeper, User approver)>
        SeedAsync(AppDbContext db)
    {
        ...
        db.Add(bookkeeper); db.Add(approver);
        await db.SaveChangesAsync();
