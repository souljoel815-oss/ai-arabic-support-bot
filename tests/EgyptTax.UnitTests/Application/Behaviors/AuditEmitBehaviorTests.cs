...
public class AuditEmitBehaviorTests
{
    ...
    public async Task AuditableRequest_HandlerSucceeds_EmitsAuditRow()
    {
        ...
        var sut = new AuditEmitBehavior<AuditedPingCommand, string>(
            store,
            user,
            FakeFirmContextResolver.Empty
        );
        ...
        await store
            ...
            .AppendAsync(
                Arg.Is<AuditLogPayload>(p =>
                    p.Kind == "AuditedPing" && p.ActorUserId == user.UserId
                ),
    ...
    {
        var store = Substitute.For<IAuditLogStore>();
        var sut = new AuditEmitBehavior<PingCommand, string>(
            store,
    ...
    public async Task AuditableRequest_FirmUserActor_OverlaysFirmName()
    {
        ...
        await store
            ...
            .AppendAsync(
                Arg.Is<AuditLogPayload>(p =>
                    p.ActorUserId == actorId && p.ActorFirmName == "Nile Accounting LLC"
                ),
                Arg.Any<CancellationToken>()
            );
    }
    ...
    public async Task AuditableRequest_InHouseActor_LeavesFirmNameNull()
    {
        ...
        await store
            .Received(1)
            .AppendAsync(
                Arg.Is<AuditLogPayload>(p => p.ActorUserId == actorId && p.ActorFirmName == null),
                Arg.Any<CancellationToken>()
            );
    }
