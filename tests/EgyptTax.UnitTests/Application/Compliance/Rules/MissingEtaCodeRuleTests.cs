...
public class MissingEtaCodeRuleTests
{
    ...
    public void Returns_NoFinding_When_All_Items_Have_Codes()
    {
        ...
        rule.Evaluate(ctx).Should().BeEmpty(
            because: "every line's item has an ETA code — nothing to flag");
    }
    ...
    public void Counts_Distinct_Items_Once_Across_Multiple_Lines()
    {
        ...
        var ctx = new DocumentRiskContext(invoice, items, null,
    ...
    private static DocumentRiskContext BuildContext(int itemsWithCodes, int itemsWithoutCodes)
    {
        ...
        return new DocumentRiskContext(invoice, items, null,
            new DateTime(2026, 5, 7, 12, 0, 0, DateTimeKind.Utc));
    }

    private static (SalesInvoice invoice, Dictionary<Guid, Item> items)
        BuildInvoiceAndItems(int itemsWithCodes, int itemsWithoutCodes)
    {
        ...
        {
            var item = new Item($"OK-{i:D2}",
                new ArabicEnglishText("صنف", "Item"), VatId, etaItemCode: "EGS1.001");
            items[item.Id] = item;
            invoice.AddLine(item.Id, 1m, MoneyEgp.From(100m), VatId, 14m);
        }
        for (var i = 0; i < itemsWithoutCodes; i++)
        {
            var item = new Item($"BAD-{i:D2}",
                new ArabicEnglishText("صنف", "Item"), VatId);
            items[item.Id] = item;
            invoice.AddLine(item.Id, 1m, MoneyEgp.From(100m), VatId, 14m);
        }
