...
public class SupplierTinRevalidationJobTests
{
    ...
    public async Task ValidSuppliers_Increment_ValidCount_NoInvalidatedEvents()
    {
        ...
        audit.Captured.Should().NotContain(e => e.Kind == "supplier.tin_invalidated",
    ...
    public async Task InvalidatedTin_Emits_PerSupplier_AuditEvent()
    {
        ...
        var source = new StubSource(new[]
        {
            new SupplierTinRow(supplierId, "999999999", "Bad Supplier"),
        });
        var job = new SupplierTinRevalidationJob(
            source, new InvalidatingRevalidator(), audit,
            new FixedClock(DateTime.UtcNow));
    ...
    public async Task PerRowException_LoggedAndDoesNotAbortBatch()
    {
        ...
        var job = new SupplierTinRevalidationJob(
            source, new ThrowsOnSecondRevalidator(), audit,
            new FixedClock(DateTime.UtcNow));
    ...
    private sealed class InvalidatingRevalidator : ISupplierTinRevalidator
    {
        public Task<TinRevalidationResult> RevalidateAsync(string tin, CancellationToken cancellationToken = default) =>
            Task.FromResult(new TinRevalidationResult(tin, IsValid: false, "stub:always-invalid", "Test invalidation"));
    }
