...
public class InspectionBundleJobTests : IDisposable
{
    ...
    public async Task HappyPath_EmitsStartedThenComplete_AndPersistsZip()
    {
        ...
        builder
            .BuildAsync(Arg.Any<InspectionBundleRequest>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(fakeBundle));

        var job = new InspectionBundleJob(
            builder,
            notifier,
        ...
        notifier
            ...
            .Should()
            .Equal(InspectionBundleProgressPhase.Started, InspectionBundleProgressPhase.Complete);

        var complete = notifier.Events[^1];
        complete.JobId.Should().Be(jobId);
        complete.ResultPath.Should().NotBeNull();
        File.Exists(complete.ResultPath!)
            .Should()
            .BeTrue(
    ...
    public async Task BuilderThrows_EmitsFailed_AndRethrowsForHangfireRetry()
    {
        ...
        builder
            .BuildAsync(Arg.Any<InspectionBundleRequest>(), Arg.Any<CancellationToken>())
            .Returns<Task<InspectionBundleResult>>(_ =>
        ...
        notifier
            .Events.Select(e => e.Phase)
            .Should()
            .Equal(InspectionBundleProgressPhase.Started, InspectionBundleProgressPhase.Failed);
    ...
    private static InspectionBundleResult BuildFakeBundle()
    {
        ...
        var manifest = new InspectionBundleManifest(
            ...
            Company: new BundleCompany(
                Guid.NewGuid(),
                "123456789",
                new BundleBilingualName("شركة", "Co")
            ),
            ...
            AuditChainExtract: new BundleAuditChainExtract(
                1,
                100,
                "genesis",
