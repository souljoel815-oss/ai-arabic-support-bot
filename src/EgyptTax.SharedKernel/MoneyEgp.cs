...
public readonly record struct MoneyEgp(decimal Amount) : IComparable<MoneyEgp>
{
    ...
    /// </summary>
    public decimal AmountRoundedToCents =>
    ...
    public static MoneyEgp operator *(MoneyEgp left, decimal scalar) =>
        new(left.Amount * scalar);

    public static MoneyEgp operator *(decimal scalar, MoneyEgp right) =>
        new(scalar * right.Amount);
