...
public sealed class InspectionBundleBuilder : IInspectionBundleBuilder
{
    ...
    )
    {
        ...
        var draftIds = await CollectDraftIdsAsync(
            request.PeriodStart,
            request.PeriodEnd,
            cancellationToken
        );
        ...
        var auditEntries = await CollectAuditEntriesAsync(
            request.PeriodStart,
            request.PeriodEnd,
            cancellationToken
        );
        ...
        using (var zip = new ZipArchive(zipStream, ZipArchiveMode.Create, leaveOpen: true))
        {
            ...
            files.Add(
                await WriteEntryAsync(
                    zip,
                    "registers/purchase-and-expense-register.pdf",
                    "PurchaseInvoiceAndExpenseRegister",
                    purchaseExpenseRegisterBytes,
                    cancellationToken
                )
            );
            ...
            files.Add(
                await WriteEntryAsync(
                    zip,
                    "registers/credit-note-and-reversal-register.pdf",
                    "CreditNoteAndReversalRegister",
                    creditNoteRegisterBytes,
                    cancellationToken
                )
            );
            ...
            foreach (
                var (attachment, contentBytes) in await ReadAttachmentBytesAsync(
                    attachments,
                    cancellationToken
                )
            )
            {
                var relativePath =
                    $"attachments/{attachment.DocumentId:D}/{attachment.FilenameStorage}";
                var entry = await WriteEntryAsync(
                    zip,
                    relativePath,
                    "Attachment",
                    contentBytes,
            ...
            var manifest = BuildManifestSkeleton(
                request,
                company,
                files,
                auditEntries,
            ...
            var manifestJsonForHash = JsonSerializer.SerializeToUtf8Bytes(
                manifest with
                {
            ...
            var finalManifestBytes = JsonSerializer.SerializeToUtf8Bytes(
                finalManifest,
                ManifestJson
            );
    ...
    private async Task<List<Guid>> CollectDraftIdsAsync(
        DateOnly start,
        DateOnly end,
        CancellationToken ct
    )
    {
        var sales = await _db.Set<SalesInvoice>()
            .AsNoTracking()
            .Where(s =>
                s.State == DocumentState.Draft && s.DocumentDate >= start && s.DocumentDate <= end
            )
            .Select(s => s.Id)
            .ToListAsync(ct);
        var purchases = await _db.Set<PurchaseInvoice>()
            .AsNoTracking()
            .Where(p =>
                p.State == DocumentState.Draft && p.DateReceived >= start && p.DateReceived <= end
            )
            .Select(p => p.Id)
            .ToListAsync(ct);
        var expenses = await _db.Set<Expense>()
            .AsNoTracking()
            .Where(e =>
                e.State == DocumentState.Draft && e.DocumentDate >= start && e.DocumentDate <= end
            )
            .Select(e => e.Id)
            .ToListAsync(ct);
    ...
    private async Task<List<Attachment>> CollectAttachmentsAsync(
        DateOnly start,
        DateOnly end,
        CancellationToken ct
    )
    {
        ...
        var purchaseIds = await _db.Set<PurchaseInvoice>()
            .AsNoTracking()
            .Where(p =>
                p.State == DocumentState.Posted && p.DateReceived >= start && p.DateReceived <= end
            )
            .Select(p => p.Id)
            .ToListAsync(ct);
        var expenseIds = await _db.Set<Expense>()
            .AsNoTracking()
            .Where(e =>
                e.State == DocumentState.Posted && e.DocumentDate >= start && e.DocumentDate <= end
            )
            .Select(e => e.Id)
            .ToListAsync(ct);
        ...
        return await _db.Set<Attachment>()
            .AsNoTracking()
    ...
    private async Task<List<AuditLogEntry>> CollectAuditEntriesAsync(
        DateOnly start,
        DateOnly end,
        CancellationToken ct
    )
    {
        ...
        return await _db.Set<AuditLogEntry>()
            .AsNoTracking()
    ...
    )
    {
        ...
        var categories = await _db.Set<DeductibleExpenseCategory>()
            .AsNoTracking()
    ...
    {
        var creditNotes = await _db.Set<SalesInvoice>()
            .AsNoTracking()
            .Where(i =>
                i.State == DocumentState.Posted
                && i.CreditNoteOfInvoiceId != null
        ...
        var origIds = creditNotes
            ...
            .Select(c => c.CreditNoteOfInvoiceId!.Value)
            .Distinct()
            .ToArray();
        var origs = await _db.Set<SalesInvoice>()
            .AsNoTracking()
    ...
    )
    {
        ...
        var entries = await _db.Set<JournalEntry>()
            ...
            .OrderBy(e => e.PostedAtUtc)
            .ThenBy(e => e.SourceDocumentNumber)
            .ToListAsync(ct);

        var rows = entries
            .Select(e => new GeneralJournalListingPdfRenderer.EntryRow(
    ...
    {
        var report = await _trialBalanceQuery.RunAsync(request.PeriodStart, request.PeriodEnd, ct);
...
#pragma warning restore CA1308
            sb.Append("{\"index\":")
                .Append(e.Index)
                .Append(",\"tsUtc\":\"")
                .Append(e.TsUtc.ToString("o", CultureInfo.InvariantCulture))
                .Append('"')
                .Append(",\"kind\":\"")
                .Append(e.Kind)
                .Append('"')
                .Append(",\"actorUserId\":")
                .Append(
                    e.ActorUserId is null ? "null" : "\"" + e.ActorUserId.Value.ToString("D") + "\""
                )
                .Append(",\"actorFirmName\":")
                .Append(
                    e.ActorFirmName is null ? "null" : "\"" + JsonEscape(e.ActorFirmName) + "\""
                )
                .Append(",\"companyId\":\"")
                .Append(e.CompanyId.ToString("D"))
                .Append('"')
                .Append(",\"prevHashHex\":\"")
                .Append(prevHex)
                .Append('"')
    ...
    )
    {
        ...
        sb.AppendLine(
            inv,
            $"- **Company**: {company.LegalName.English} ({company.LegalName.Arabic})"
        );
        sb.AppendLine(inv, $"- **TIN**: {company.TaxRegistrationNumber}");
        sb.AppendLine(
            inv,
            $"- **Period**: {request.PeriodStart:yyyy-MM-dd} → {request.PeriodEnd:yyyy-MM-dd}"
        );
        ...
        sb.AppendLine();
        sb.AppendLine(
            "1. Read `MANIFEST.sha256` to discover every file in the bundle and its expected SHA-256."
        );
        sb.AppendLine(
            "2. Recompute SHA-256 over each file (PowerShell: `Get-FileHash -Algorithm SHA256`) and compare."
        );
        sb.AppendLine(
            "3. Recompute the top-level archive hash by serialising the manifest with `topLevelArchiveSha256` set to `\"\"`,"
        );
        sb.AppendLine(
            "   then SHA-256 over those bytes; compare to the manifest's `topLevelArchiveSha256` field."
        );
        sb.AppendLine(
            "4. Replay `audit-trail/audit-trail-extract.jsonl` line-by-line: each entry's `thisHashHex` MUST equal"
        );
        ...
        sb.AppendLine("```");
        sb.AppendLine(
            "The script verifies per-file SHA-256 against the manifest. Top-level archive hash + audit-chain replay require the .NET-based verifier shipped with the application (`EgyptTax.Web verify-audit`)."
        );
        sb.AppendLine();
        ...
        sb.AppendLine(
            "- `audit-trail/audit-trail-extract.jsonl` — every audit entry whose timestamp falls inside the period."
        );
        sb.AppendLine(
            "- `attachments/{document_id}/{filename_storage}` — every attachment for posted documents in the period."
        );
        sb.AppendLine();
        if (draftIds.Count > 0)
        {
            ...
            sb.AppendLine(
                inv,
                $"The operator chose to generate this bundle while {draftIds.Count} draft document(s) dated in"
            );
            sb.AppendLine(
                "the period existed. Those drafts are NOT included in the bundle and are listed by id in"
            );
            sb.AppendLine(
                "the manifest's `excludedDraftIds` field. Inspector should request the operator to post or"
            );
            sb.AppendLine(
                "void those drafts and re-issue a fresh bundle if a complete view is required."
            );
        }
        return Encoding.UTF8.GetBytes(sb.ToString());
    }
    ...
    )
    {
        ...
        return new InspectionBundleManifest(
            ...
            Company: new BundleCompany(
                ...
                LegalName: new BundleBilingualName(
