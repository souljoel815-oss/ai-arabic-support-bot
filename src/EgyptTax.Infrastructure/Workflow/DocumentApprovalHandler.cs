...
public sealed class DocumentApprovalHandler
{
    ...
    public async Task<ApprovalRequest> SubmitAsync(SubmitDocumentCommand command, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
        var nowUtc = _clock.UtcNow;

        await TransitionAsync(command.DocumentId, command.DocumentType,
        ...
        var request = new ApprovalRequest(
            command.DocumentId, command.DocumentType,
            command.SubmittedByUserId, nowUtc);
        _db.Add(request);
    ...
    public async Task<ApprovalRequest> ApproveAsync(ApproveDocumentCommand command, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
        var nowUtc = _clock.UtcNow;
    ...
    public async Task<ApprovalRequest> RejectAsync(RejectDocumentCommand command, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
    ...
    public async Task VoidAsync(VoidDocumentCommand command, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
    ...
    private async Task<ApprovalRequest> LoadPendingApprovalAsync(Guid documentId, CancellationToken cancellationToken)
    {
        ...
        var request = await _db.Set<ApprovalRequest>()
        ...
        CancellationToken cancellationToken)
    {
        switch (type)
        {
            ...
            {
                var doc = await _db.Set<SalesInvoice>()
                    .FirstOrDefaultAsync(s => s.Id == documentId, cancellationToken)
                    ?? throw new InvalidOperationException($"Sales invoice {documentId} not found.");
                sales(doc);
                break;
            }
            case DocumentType.PurchaseInvoice:
            {
                var doc = await _db.Set<PurchaseInvoice>()
                    .FirstOrDefaultAsync(p => p.Id == documentId, cancellationToken)
                    ?? throw new InvalidOperationException($"Purchase invoice {documentId} not found.");
                purchase(doc);
                break;
            }
            case DocumentType.Expense:
            {
                var doc = await _db.Set<Expense>()
