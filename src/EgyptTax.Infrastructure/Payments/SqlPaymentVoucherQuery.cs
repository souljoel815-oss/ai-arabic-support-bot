...
public sealed class SqlPaymentVoucherQuery : IPaymentVoucherQuery
{
    ...
    )
    {
        ...
        {
            var p = await _db.Set<PurchaseInvoice>()
                .AsNoTracking()
        ...
        {
            var s = await _db.Set<SalesInvoice>()
                .AsNoTracking()
                .FirstOrDefaultAsync(x => x.Id == invoiceId, cancellationToken);
            if (s is null)
                return null;
        ...
        var allocations = await _db.Set<PaymentAllocation>()
            .AsNoTracking()
            .Where(a => a.TargetDocumentId == invoiceId)
            .ToListAsync(cancellationToken);

        // Resolve voucher document numbers + dates for the allocation rows.
        var spvIds = allocations
            .Where(a => a.SupplierPaymentVoucherId.HasValue)
            .Select(a => a.SupplierPaymentVoucherId!.Value)
            .Distinct()
            .ToArray();
        var crvIds = allocations
            .Where(a => a.CustomerReceiptVoucherId.HasValue)
            .Select(a => a.CustomerReceiptVoucherId!.Value)
            .Distinct()
            .ToArray();

        var spvLookup = await _db.Set<SupplierPaymentVoucher>()
            .AsNoTracking()
            .Where(v => spvIds.Contains(v.Id))
            .ToDictionaryAsync(
                v => v.Id,
                v => (v.DocumentNumber, v.PaymentDate),
                cancellationToken
            );
        var crvLookup = await _db.Set<CustomerReceiptVoucher>()
            .AsNoTracking()
            .Where(v => crvIds.Contains(v.Id))
            .ToDictionaryAsync(
                v => v.Id,
                v => (v.DocumentNumber, v.ReceiptDate),
                cancellationToken
            );
        ...
        var rows = allocations
            .Select(a =>
            {
                ...
                if (a.SupplierPaymentVoucherId is { } sId && spvLookup.TryGetValue(sId, out var sv))
                {
                    ...
                    a.CustomerReceiptVoucherId is { } cId
                    && crvLookup.TryGetValue(cId, out var cv)
                )
                {
    ...
    )
    {
        ...
        return seed.Select(s =>
            ...
            .Where(r => r.OpenBalance.Amount > 0m)
            .ToList();
    }
}
