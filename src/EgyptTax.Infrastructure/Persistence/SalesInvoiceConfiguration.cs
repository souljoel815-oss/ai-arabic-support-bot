...
internal sealed class SalesInvoiceConfiguration : IEntityTypeConfiguration<SalesInvoice>
{
    public void Configure(EntityTypeBuilder<SalesInvoice> b)
    {
        ...
        b.Property(s => s.DocumentDate).HasColumnName("document_date").HasColumnType("date").IsRequired();
        b.Property(s => s.State).HasColumnName("state").HasConversion<string>().HasMaxLength(16).IsRequired();

        b.Property(s => s.DocumentNumber).HasColumnName("document_number").HasMaxLength(32).IsUnicode(false);
        ...
        b.ComplexProperty(s => s.CustomerTaxProfileSnapshot, t =>
        {
            ...
            t.Property(x => x.TinValue).HasColumnName("customer_tax_profile_snapshot_tin")
                .HasMaxLength(9).IsUnicode(false);
            t.Property(x => x.VatExemption).HasColumnName("customer_tax_profile_snapshot_vat_exemption").IsRequired();
