...
internal sealed class ApprovalRequestConfiguration : IEntityTypeConfiguration<ApprovalRequest>
{
    public void Configure(EntityTypeBuilder<ApprovalRequest> b)
    {
        ...
        b.Property(r => r.DocumentType).HasColumnName("document_type")
            .HasConversion<string>().HasMaxLength(32).IsRequired();
        b.Property(r => r.SubmittedByUserId).HasColumnName("submitted_by_user_id").IsRequired();
        b.Property(r => r.SubmittedAtUtc).HasColumnName("submitted_at_utc").HasColumnType("datetime2(3)").IsRequired();

        b.Property(r => r.ApprovedByUserId).HasColumnName("approved_by_user_id");
        b.Property(r => r.ApprovedAtUtc).HasColumnName("approved_at_utc").HasColumnType("datetime2(3)");
        b.Property(r => r.RejectedByUserId).HasColumnName("rejected_by_user_id");
        b.Property(r => r.RejectedAtUtc).HasColumnName("rejected_at_utc").HasColumnType("datetime2(3)");
        ...
        b.HasIndex(r => r.SubmittedByUserId).HasDatabaseName("ix_approval_requests_submitted_by_user_id");

        // Status is derived; the queue query orders by SubmittedAtUtc
        // among Pending rows (Approved+Rejected nullable both null).
        b.HasIndex(r => r.SubmittedAtUtc).HasDatabaseName("ix_approval_requests_submitted_at_utc");
    }
}
