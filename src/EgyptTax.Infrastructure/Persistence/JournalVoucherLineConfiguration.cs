...
internal sealed class JournalVoucherLineConfiguration : IEntityTypeConfiguration<JournalVoucherLine>
{
    public void Configure(EntityTypeBuilder<JournalVoucherLine> b)
    {
        ...
        b.Property(l => l.AccountCode).HasColumnName("account_code").HasMaxLength(16).IsUnicode(false).IsRequired();
        b.Property(l => l.Description).HasColumnName("description").HasMaxLength(256).IsRequired();

        b.ComplexProperty(l => l.Debit,
            p => p.Property(x => x.Amount).HasColumnName("debit").HasColumnType("decimal(19,2)").IsRequired());
        b.ComplexProperty(l => l.Credit,
            p => p.Property(x => x.Amount).HasColumnName("credit").HasColumnType("decimal(19,2)").IsRequired());

        b.HasIndex(l => l.JournalVoucherId).HasDatabaseName("ix_journal_voucher_lines_journal_voucher_id");
