...
internal sealed class Form41FilingConfiguration : IEntityTypeConfiguration<Form41Filing>
{
    public void Configure(EntityTypeBuilder<Form41Filing> b)
    {
        ...
        b.Property(f => f.GeneratedAtUtc).HasColumnName("generated_at_utc").HasColumnType("datetime2(3)").IsRequired();
        ...
        b.Property(f => f.StructuredJsonPath).HasColumnName("structured_json_path").HasMaxLength(500).IsRequired();
        b.Property(f => f.LineCount).HasColumnName("line_count").IsRequired();

        b.ComplexProperty(f => f.TotalWhtPayable,
            p => p.Property(x => x.Amount).HasColumnName("total_wht_payable").HasColumnType("decimal(19,2)").IsRequired());
