...
public sealed class SqlJournalLedgerQuery : IJournalLedgerQuery
{
    ...
    )
    {
        ...
        // query per entry.
        var autoEntries = await _db.Set<JournalEntry>()
            .AsNoTracking()
            .Include(e => e.Lines)
        ...
        // the right period.
        var vouchers = await _db.Set<JournalVoucher>()
            .AsNoTracking()
            .Include(v => v.Lines)
        ...
        var voucherRows = vouchers.Select(v => new JournalListRow(
            Id: v.Id,
            Kind: v.ReversesJournalVoucherId is not null
    ...
    )
    {
        if (kind == JournalEntryKind.AutoEmitted)
        {
            ...
            return new JournalDetailDto(
                ...
                Lines: entry
                    .Lines.Select(l => new JournalDetailLineDto(
                        l.AccountCode,
                        l.Debit.Amount,
                        l.Credit.Amount,
                        l.Description
        ...
        var voucher = await _db.Set<JournalVoucher>()
            .AsNoTracking()
            .Include(v => v.Lines)
