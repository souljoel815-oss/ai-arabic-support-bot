...
public sealed class CreateReversalJournalHandler
{
    ...
    public CreateReversalJournalHandler(AppDbContext db, IClock clock, IAuditLogStore auditLog)
    {
        _db = db;
        _clock = clock;
        _auditLog = auditLog;
    ...
    )
    {
        ...
        var original =
            await _db.Set<JournalVoucher>()
                .Include(v => v.Lines)
                .FirstOrDefaultAsync(
                    v => v.Id == command.OriginalJournalVoucherId,
                    cancellationToken
                )
            ?? throw new InvalidOperationException(
        ...
        var alreadyReversed = await _db.Set<JournalVoucher>()
            .AsNoTracking()
