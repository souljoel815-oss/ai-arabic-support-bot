...
public sealed class SqlTaxableIncomeReportQuery : ITaxableIncomeReportQuery
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var categoryNames = await _db.Set<DeductibleExpenseCategory>().AsNoTracking()
        ...
        {
            rows.Add(new TaxableIncomeRow(
                Bucket: BucketRevenue,
                DocumentId: s.Id,
                DocumentType: s.IsCreditNote ? DocumentType.CreditNote : DocumentType.SalesInvoice,
                DocumentNumber: s.DocumentNumber,
                DocumentDate: s.DocumentDate,
                Description: customerNames.GetValueOrDefault(s.CustomerId, s.CustomerId.ToString("D")),
        ...
        var purchaseGrouped = purchaseLines
            .GroupBy(r => new { r.Id, r.DocumentNumber, r.DateReceived, r.SupplierId, r.DeductibleFlag })
            .Select(g => new
            {
                g.Key.Id, g.Key.DocumentNumber, g.Key.DateReceived,
                g.Key.SupplierId, g.Key.DeductibleFlag,
        ...
        foreach (var p in purchaseGrouped)
        {
            rows.Add(new TaxableIncomeRow(
                ...
                Description: supplierNames.GetValueOrDefault(p.SupplierId, p.SupplierId.ToString("D"))
        ...
        var deductibleFromPurchases = purchaseLines.Where(l => l.DeductibleFlag).Sum(l => l.LineSubtotal);
        var deductibleFromExpenses = expenseRows.Where(e => e.DeductibleFlag).Sum(e => e.Amount);
        var deductible = decimal.Round(deductibleFromPurchases + deductibleFromExpenses, 2, MidpointRounding.ToEven);
        var nonDeductibleFromPurchases = purchaseLines.Where(l => !l.DeductibleFlag).Sum(l => l.LineSubtotal);
        var nonDeductibleFromExpenses = expenseRows.Where(e => !e.DeductibleFlag).Sum(e => e.Amount);
        var nonDeductible = decimal.Round(nonDeductibleFromPurchases + nonDeductibleFromExpenses, 2, MidpointRounding.ToEven);

        var managementPL = decimal.Round(revenue - deductible - nonDeductible, 2, MidpointRounding.ToEven);
