    ...
    public static byte[] Render(WhtCertificatePayload payload)
    {
        ...
        var doc = Document.Create(c =>
        {
            c.Page(page =>
            {
                ...
                {
                    col.Item().AlignCenter().Text("WHT Certificate / شهادة خصم").Bold().FontSize(16);
                ...
                page.Content().PaddingVertical(10).Column(col =>
                {
                    ...
                    col.Item().Row(row =>
                    {
                        ...
                        {
                            right.Item().AlignRight().Text("Counterparty / الطرف الآخر").Bold().FontSize(11);
                            right.Item().AlignRight().Text(payload.CounterpartyName.En);
                            right.Item().AlignRight().Text(payload.CounterpartyName.Ar).FontSize(9);
                            right.Item().AlignRight().Text($"TIN: {payload.CounterpartyTin}");
                        });
                    });
                    ...
                    // Source documents.
                    col.Item().PaddingTop(10).Text("Source documents / المستندات الأصلية").Bold().FontSize(11);
                    col.Item().Row(row =>
                    {
                        row.RelativeItem().Column(left =>
                        {
                            left.Item().Text($"Invoice no.: {payload.SourceInvoiceNumber}");
                            left.Item().Text(
                                $"Invoice date: {payload.SourceInvoiceDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)}");
                        });
                        row.RelativeItem().AlignRight().Column(right =>
                        {
                            right.Item().AlignRight().Text($"Voucher no.: {payload.SourceVoucherNumber}");
                    ...
                    // Withholding details.
                    col.Item().PaddingTop(10).Text("Withholding details / تفاصيل الخصم").Bold().FontSize(11);
                    ...
                    col.Item().PaddingTop(20).Row(row =>
                    {
                        ...
                        {
                            right.Item().AlignRight().Text("Authorised signature / توقيع معتمد").FontSize(9);
