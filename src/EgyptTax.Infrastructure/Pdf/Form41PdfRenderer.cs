    ...
    public static byte[] Render(Form41Payload payload)
    {
        ...
        var doc = Document.Create(c =>
        {
            c.Page(page =>
            {
                ...
                {
                    col.Item().AlignCenter().Text("Form 41 — Quarterly WHT Return / نموذج 41 — إقرار الخصم الفصلي")
                        .Bold().FontSize(14);
        ...
        container.Table(table =>
        {
            ...
            foreach (var line in payload.Lines)
            {
                ...
                table.Cell().AlignRight().Text(
                    line.RateAppliedPercent.ToString("0.##", CultureInfo.InvariantCulture));
                table.Cell().AlignRight().Text(
                    line.GrossPaymentTotal.ToString("F2", CultureInfo.InvariantCulture));
                table.Cell().AlignRight().Text(
                    line.AmountWithheld.ToString("F2", CultureInfo.InvariantCulture));
                table.Cell().Text(line.SupplierPaymentVoucherNumber).FontSize(8);
                table.Cell().Text(
                    line.SupplierPaymentVoucherDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)).FontSize(8);
    ...
    private static void RenderReconciliation(IContainer container, Form41Payload payload)
    {
        container.AlignRight().Column(col =>
        {
            ...
            col.Item().PaddingTop(6).AlignRight().Text(
                ...
                .FontSize(7);
            col.Item().AlignRight().Text(
                $"hash {payload.AuditChainExtractRef.ExtractSha256[..16]}…")
                .FontSize(7);
        });
    }
}
