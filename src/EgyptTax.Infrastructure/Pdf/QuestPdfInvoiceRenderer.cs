...
public sealed class QuestPdfInvoiceRenderer : ISalesInvoicePdfRenderer
{
        ...
        var doc = QuestPDF.Fluent.Document.Create(c =>
        {
            c.Page(page =>
            {
                ...
                page.Header().Column(col =>
                {
                    col.Item().Row(row =>
                    {
                        row.RelativeItem().Column(left =>
                        {
                            ...
                            left.Item().Text($"CR: {request.Issuer.CommercialRegistrationNumber}");
                        ...
                        row.RelativeItem().AlignRight().Column(right =>
                        {
                            ...
                            right.Item().AlignRight().Text($"No. {invoice.DocumentNumber}");
                ...
                page.Content().PaddingVertical(15).Column(col =>
                {
                    ...
                    if (invoice.CustomerTaxProfileSnapshot.ProfileType == CustomerTaxProfileType.B2BRegistered
                    ...
                    col.Item().PaddingTop(20).Row(row =>
                    {
                        ...
                        {
                            left.Item().Text(
                                $"Posted by: {request.PostedByUserDisplayName}");
                            ...
                            left.Item().Text("Amounts rounded to two decimal places / الأرقام مقربة لأقرب قرشين")
                                .FontSize(8);
                        });
                        row.ConstantItem(120).AlignRight().Column(right =>
                        {
                            right.Item().AlignRight().Image(qrPng).FitArea();
                            right.Item().AlignRight().Text("Verification seal").FontSize(8);
                        });
                    });
                });
    ...
    private static void RenderLines(IContainer container, InvoicePdfRequest request)
    {
        container.Table(table =>
        {
            ...
            {
                var itemCode = request.Items.TryGetValue(line.ItemId, out var item) ? item.Code : line.ItemId.ToString("N");
                var itemNameEn = request.Items.TryGetValue(line.ItemId, out item) ? item.Name.English : itemCode;
                var itemNameAr = request.Items.TryGetValue(line.ItemId, out item) ? item.Name.Arabic : "";
                ...
                table.Cell().AlignRight().Text(line.LineTotal.Amount.ToString("F2", CultureInfo.InvariantCulture));
