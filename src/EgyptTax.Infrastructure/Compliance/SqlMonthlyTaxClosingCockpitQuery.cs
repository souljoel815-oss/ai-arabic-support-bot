...
public sealed class SqlMonthlyTaxClosingCockpitQuery : IMonthlyTaxClosingCockpitQuery
{
    ...
    public async Task<MonthlyTaxClosingCockpit> RunAsync(int year, int month, CancellationToken cancellationToken = default)
    {
        ...
        var periodRow = await _db.Set<TaxPeriod>().AsNoTracking()
            .FirstOrDefaultAsync(p => p.PeriodKind == TaxPeriodKind.VatMonth
                && p.Year == year && p.MonthOrQuarter == month, cancellationToken);
        ...
        // SalesInvoice table).
        var postedSales = await _db.Set<SalesInvoice>().AsNoTracking()
            .Where(s => s.State == DocumentState.Posted
                && s.DocumentDate >= periodStart && s.DocumentDate <= periodEnd)
            .Select(s => new { s.Id, s.DocumentNumber, s.DocumentDate, s.IsCreditNote, s.CustomerId })
        ...
        var postedPurchases = await (
            ...
            where p.State == DocumentState.Posted
                && p.DateReceived >= periodStart && p.DateReceived <= periodEnd
            select new
            {
                p.Id,
                p.DocumentNumber,
                ...
                InputVat = p.Lines.Where(l => l.DeductibleFlag).Sum(l => (decimal?)l.LineVat.Amount) ?? 0m,
            }).ToListAsync(cancellationToken);

        // Posted expenses in period.
        var postedExpenses = await _db.Set<Expense>().AsNoTracking()
            .Where(e => e.State == DocumentState.Posted
                && e.DocumentDate >= periodStart && e.DocumentDate <= periodEnd)
            .Select(e => new { e.Id, e.DocumentNumber, e.DocumentDate, e.DeductibleFlag, e.CategoryId })
            .ToListAsync(cancellationToken);

        // Drafts in period — anything in Draft state with a date in the
        // window. Operator must post or void these before locking.
        var draftSalesCount = await _db.Set<SalesInvoice>().AsNoTracking()
            .CountAsync(s => s.State == DocumentState.Draft
                && s.DocumentDate >= periodStart && s.DocumentDate <= periodEnd, cancellationToken);
        var draftPurchaseCount = await _db.Set<PurchaseInvoice>().AsNoTracking()
            .CountAsync(p => p.State == DocumentState.Draft
                && p.DateReceived >= periodStart && p.DateReceived <= periodEnd, cancellationToken);
        var draftExpenseCount = await _db.Set<Expense>().AsNoTracking()
            .CountAsync(e => e.State == DocumentState.Draft
                && e.DocumentDate >= periodStart && e.DocumentDate <= periodEnd, cancellationToken);
        ...
        var documentIds = deductiblePurchases.Select(p => p.Id)
            .Concat(deductibleExpenses.Select(e => e.Id))
            .ToArray();
        var attachmentCounts = documentIds.Length == 0
            ? new Dictionary<Guid, int>()
            : await _db.Set<Attachment>().AsNoTracking()
        ...
        var etaRows = salesIds.Length == 0
            ? new List<EtaSubmission>()
            : await _db.Set<EtaSubmission>().AsNoTracking()
        ...
        var customerNames = customerIds.Length == 0
            ...
            : await _db.Set<Customer>().AsNoTracking()
        ...
        var supplierNames = supplierIds.Length == 0
            ? new Dictionary<Guid, string>()
            : await _db.Set<Supplier>().AsNoTracking()
        ...
        {
            missingBuckets.Add(new MissingDocumentBucket(
                Name: "Posted purchase invoices with deductible lines but no attachment (FR-016)",
                Count: purchasesMissingAttachment.Count,
                Examples: purchasesMissingAttachment.Take(ExamplesPerBucket)
                    .Select(p => new MissingDocumentExample(
                        p.Id, "PurchaseInvoice", p.DocumentNumber, p.DateReceived,
                        supplierNames.GetValueOrDefault(p.SupplierId, p.SupplierId.ToString("D"))))
                    .ToList()));
        }
        if (expensesMissingAttachment.Count > 0)
        {
            missingBuckets.Add(new MissingDocumentBucket(
                ...
                Examples: expensesMissingAttachment.Take(ExamplesPerBucket)
                    .Select(e => new MissingDocumentExample(
                        e.Id, "Expense", e.DocumentNumber, e.DocumentDate,
        ...
        if (salesMissingEtaSubmission.Count > 0)
        {
            missingBuckets.Add(new MissingDocumentBucket(
                ...
                Examples: salesMissingEtaSubmission.Take(ExamplesPerBucket)
                    .Select(s => new MissingDocumentExample(
                        s.Id, s.IsCreditNote ? "CreditNote" : "SalesInvoice",
                        s.DocumentNumber, s.DocumentDate,
                        customerNames.GetValueOrDefault(s.CustomerId, s.CustomerId.ToString("D"))))
                    .ToList()));
        }
        ...
        var failedTotal = failedSalesIds.Length == 0
            ? 0m
            : await _db.Set<SalesInvoice>().AsNoTracking()
        ...
        var dirtyPostIds = new HashSet<Guid>(
            purchasesMissingAttachment.Select(p => p.Id)
        ...
        var readiness = totalPosts == 0
            ? 100m
            : decimal.Round((decimal)cleanPosts / totalPosts * 100m, 1, MidpointRounding.ToEven);
        ...
        {
            new("All drafts dated in period are posted or voided",
                Cleared: totalDrafts == 0,
                RelatedCount: totalDrafts == 0 ? null : totalDrafts),
            new("All deductible posts have at least one attachment (FR-016)",
                Cleared: purchasesMissingAttachment.Count == 0 && expensesMissingAttachment.Count == 0,
                RelatedCount: purchasesMissingAttachment.Count + expensesMissingAttachment.Count is var c && c == 0 ? null : c),
            new("All posted sales invoices have an ETA submission row",
                Cleared: salesMissingEtaSubmission.Count == 0,
                RelatedCount: salesMissingEtaSubmission.Count == 0 ? null : salesMissingEtaSubmission.Count),
            new("No Failed ETA submissions outstanding for the period",
                Cleared: failedEta.Count == 0,
                RelatedCount: failedEta.Count == 0 ? null : failedEta.Count),
            new("Period not yet locked (FR-037)",
                Cleared: !isLocked,
                RelatedCount: null),
        };
